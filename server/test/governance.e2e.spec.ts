...
describe('举报治理真实 HTTP / PostgreSQL 闭环', () => {
  ...
  it('评论下架同步数量、采纳状态、互动和作者资料，期限结束按当前状态恢复', async () => {
    ...
    expect((await request(`/community/posts/${row.id}/comments`, reporter)).data.items.some((c: { id: string }) => c.id === comment.id)).toBe(false)
    ...
    const masked = (await request(`/community/posts/${row.id}/comments`, reporter)).data.items.find((c: { id: string }) => c.id === comment.id)
    ...
    const restored = (await request(`/community/posts/${row.id}/comments`, reporter)).data.items.find((c: { id: string }) => c.id === comment.id)
  ...
  it('申诉可撤销具体内容复核，新修订与新规则均不能被旧申诉放行', async () => {
    ...
    const review = await db.contentReview.create({ data: { targetType: 'post', targetId: row.id, postId: row.id, authorId: author.id, submittedById: author.id, contentRevision: row.revision, ruleVersion: policy.data.version, status: 'rejected', reviewedById: moderator.id, reason: '合成待审样本', action: 'review', findings: {} } })
    ...
    const next = await db.contentReview.create({ data: { targetType: 'post', targetId: valid.id, postId: valid.id, authorId: author.id, submittedById: author.id, contentRevision: 1, ruleVersion: policy.data.version, status: 'rejected', reviewedById: moderator.id, reason: '合成引用需核查', action: 'review', findings: {} } })
