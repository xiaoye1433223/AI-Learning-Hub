...
describe('学习社区视觉契约', () => {
  ...
  it('样式入口只加载分层文件，基础重置、复选框和深色工作台迁移完整', () => {
    ...
    expect(source('styles/components.css')).toContain('label:has(> input[type="checkbox"]) { display: flex; align-items: flex-start')
  ...
  it('引导页资料字段独立对齐，主题复选与移动单列不依赖弹层样式', () => {
    ...
    expect(style).toContain('.onboarding-themes label { padding: 16px; border: 1px solid var(--amc-border)')
    expect(source('styles/components.css')).toContain('min-height: 16px; margin: 3px 0 0; padding: 0; accent-color: var(--amc-orange)')
