...
describe('统一登录恢复', () => {
  ...
  it('Mock 注册保留用户选择的规范化账号并进入未认证只读态', async () => {
    const auth = useAuthStore()
    await auth.register({ username: ' Student_2026 ', displayName: '演示同学', email: 'DEMO@EXAMPLE.INVALID', password: 'ValidCampus82!', agreementVersion: 'v1' })
    ...
    await expect(auth.register({ username: 'admin', displayName: '演示同学', email: 'demo@example.invalid', password: 'ValidCampus82!', agreementVersion: 'v1' })).rejects.toThrow('账号不可用')
  })
})
